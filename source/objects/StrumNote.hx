...
class StrumNote extends FlxSprite
{
	...
	public function reloadNote()
	{
		...
		if(PlayState.isPixelStage)
		{
			...
			animation.add(colArray[noteData], [noteData + 4]);
			animation.add('static', [noteData]);
			animation.add('pressed', [noteData + 4, noteData + 8], 12, false);
			animation.add('confirm', [noteData + 12, noteData + 16], 12, false);
		}
		else
		{
			...
			animation.addByPrefix('pressed', dirArray[noteData].toLowerCase() + ' press', 24, false);
			animation.addByPrefix('confirm', dirArray[noteData].toLowerCase() + ' confirm', 24, false);
		}
		updateHitbox();
