...
class PlayState extends MusicBeatState
{
	...
	private function keyShit():Void
	{
		...
		if (!char.stunned && generatedMusic)
		{
			...
			if (controlHoldArray.contains(true) && !endingSong && opponentChart) {/*bruh*/} else if (dad.holdTimer > Conductor.stepCrochet * 0.001 * dad.singDuration && dad.animation.curAnim.name.startsWith('sing') && !dad.animation.curAnim.name.endsWith('miss'))
				dad.dance();
		}
